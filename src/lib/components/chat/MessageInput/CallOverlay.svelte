			...
			>
				...
				{:else}
					<div
						...
						style={`background-image: url('${WEBUI_API_BASE_URL}/models/model/profile/image?id=${model?.id}&lang=${$i18n.language}&size=128');`}
					/>
				{/if}
				<!-- navbar -->
			</button>
				...
				>
					...
					{:else}
						<div
							...
							style={`background-image: url('${WEBUI_API_BASE_URL}/models/model/profile/image?id=${model?.id}&lang=${$i18n.language}&size=512');`}
						/>
					{/if}
				</button>
			{:else}
